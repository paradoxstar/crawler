...
storename = 'zufangxiaoqu_guangzhou'# + time.strftime("%Y_%m_%d_%X", time.localtime())
...
def area_xiaoqu_search(db_xq,region=u"tianhe"):
    ...
    trytimes = 0
    url=u"https://gz.lianjia.com/zufang/"+region+"/"
    while 1:
        try:
    ...
    xqtotal_num = int(soup.find('div',{'class':'list-head clear'}).h2.span.text.strip())

    print u"开始爬 %s 区全部的租房小区信息" % region
    ...
    for i in range(total_pages):
        url_page = u"https://gz.lianjia.com/zufang/%s/pg%d/" % (region, i + 1)
